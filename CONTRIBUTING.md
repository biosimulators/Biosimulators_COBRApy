...
BioSimulators-COBRApy is documented using [reStructuredText](https://www.sphinx-doc.org/en/master/usage/restructuredtext/index.html) and the [napoleon Sphinx plugin](https://www.sphinx-doc.org/en/master/usage/extensions/napoleon.html). The documentation can be compiled by running the following commands:
...
3. Increment the `__version__` variable in `biosimulators_cobrapy/_version.py`.
4. Commit this change to `biosimulators_cobrapy/_version.py`.
5. Add a tag for the new version by running `git tag { version }`. `version` should be equal to the value of the
   `__version__` variable in `biosimulators_cobrapy/_version.py`.
